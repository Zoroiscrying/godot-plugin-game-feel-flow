...
func _show_preview_window() -> void:
	...
	_preview_window.add_child(_preview_control)
...
func _create_toolbar() -> Control:
	...
	toolbar.add_child(target_label)
	
	var target_option = OptionButton.new()
	target_option.add_item("Sprite2D")
	target_option.add_item("ColorRect")
	target_option.add_item("MeshInstance3D")
	toolbar.add_child(target_option)
